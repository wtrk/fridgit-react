...
export default function FullWidthTabs() {
  ...
  function handlechange_modal(event) {
    ...
    fetch("http://localhost/company/ws/ws_country", requestOptions)
...

    {
      ...
      options: {
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          return (
            <div>
              <Link to="/admin/LiveTransportation_visit">View</Link>
            </div>
          );
        },
      },
  ...
  function deleterow(row) {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          "http://localhost/company/ws/ws_tfridges.php?action=2",
          requestOptions
        )
          .then((response) => response.json())
          .then((data) => {
  ...
  useEffect(() => {
    fetch("http://localhost/company/api/ws_city")
      .then((response) => response.json())
      .then((data) => {
        //  alert("use");
        setItems(data);
  ...
  useEffect(() => {
    fetch("http://localhost/company/ws/ws_clients")
      .then((response) => response.json())
      .then((data) => {
        const listclients = data.map((data) => (
